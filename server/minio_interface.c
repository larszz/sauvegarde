...
// MINIO SETTINGS
// #define MINIO_BUCKETNAME_DEFAULT "testbucket"
#define MINIO_HOSTNAME_DEFAULT "localhost:9000"
#define MINIO_ACCESSKEY_DEFAULT "minio"
#define MINIO_SECRETKEY_DEFAULT "klav_klinke_lampe"
...
static const char *accessKeyIdG = NULL;
static const char *secretAccessKeyG = NULL;
...
// util ----------------------------------------------------------------------
/**
 * simple s3 initialization, with the default hostname; no access key set -> new / changed methods will not work
 */
static void S3_init_default()
{
    S3Status status;

    // Hostname
    const char *hostname = MINIO_HOSTNAME_DEFAULT;

    if ((status = S3_initialize(NULL, verifyPeerG | S3_INIT_ALL, hostname))
        != S3StatusOK)
    {
...
/**
 * Initializes libs3, stores the access keys in local variables
 * @param hostname
 * @param access_key
 * @param secret_key
 */
bool S3_init_with_parameters(const char *hostname, const char *access_key, const char *secret_key)
{
    S3Status status;

    // Hostname
    if (!hostname)
        hostname = MINIO_HOSTNAME_DEFAULT;
    ...
    if (access_key)
        ...
        accessKeyIdG = MINIO_ACCESSKEY_DEFAULT;
    ...
    if (secret_key)
        ...
        secretAccessKeyG = MINIO_SECRETKEY_DEFAULT;
...
// test bucket ---------------------------------------------------------------
/**
 * Tests, if a bucket can be accessed by the passed bucket name
 * @param bucketName is the Bucket to search for
 * @return if bucket can be accessed
 * (Reasons for FALSE:
 * - param 'bucketName' not set
 * - Bucket doesn't exist
 * - Access to found bucket is denied
 */
bool s3_test_bucket(const char *bucketName)
{
    bool access = 0;

    // Return if bucketname_data not set
    if (!bucketName)
    {
        fprintf(stderr, "%s: parameter %s not set!\n", "s3_test_bucket", "bucketName");
        return false;
    }
...
bool create_bucket(const char *bucketName, bool validateBucketName)
{
    const char *locationConstraint = 0;
    S3CannedAcl cannedAcl = S3CannedAclPrivate;


    if (validateBucketName && (S3_validate_bucket_name
...
static int putObjectDataCallback(int bufferSize, char *buffer,
                                 void *callbackData)
{
    // convert passed callbackData to an actual callback data
    ...
    if (data->contentLength)
    {
        ...
        if (ret <= 0)
        {
            int eof = feof(data->infile);
            int err = ferror(data->infile);
            fprintf(stdout, "\nRet is 0! feof: %d ; ferror: %d\n", eof, err);
        }
    }
...
/**
 * Puts passed data (1. File data, 2. passed char* data) to the specified bucket with passed key
 * @param targetBucket
 * @param targetKey
 * @param srcFilename
 * @param dataToWrite
 * @TODO IMPORTANT: add boolean return value, make always return the right success val if error happened or not
 */
void put_object(const char *targetBucket, const char *targetKey, const char *dataToWrite)
{
    const char *key = targetKey;
    const char *uploadId = 0;
    // const char *filename = srcFilename;
    ...
    // NO PASSED FILE PATH ACCEPTED ANYMORE
    /*
    if (filename)
    {
        if (!contentLength)
        ...
        // else check for possible input data to use instead of file data
    else
    { */
    if (dataToWrite)
    {
        int numChars;
        size_t size;
        char result[strlen(dataToWrite) + 1];
        char *MemStrPtr;


        // write data to stream
        data.infile = open_memstream(&MemStrPtr, &size);
        numChars = fprintf(data.infile, "%s", dataToWrite);
        if (numChars <= 0)
        {
            fprintf(stderr, "\nERROR: while fprintf to other file!");
            perror(0);
            exit(-1);
        }

        // set contentLength
        contentLength = numChars;

        // TODO CHECK: can MemStrPtr already be freed here?  Or where should we do that?!  Maybe at the end of the method, after the file has been closed?
    }

        // neither filename nor data to write passed
    else
    {
        fprintf(stderr, "\nERROR: No filename and no direct data set!");
        perror(0);
        exit(-1);
    }
//    }
    ...
    if (contentLength <= MULTIPART_CHUNK_SIZE)
    {
        ...
        // MULTIPART necessary, because contentsize bigger than chunk size
    else
    {
        ...
        if (uploadId)
        {
            ...
            if (!try_get_parts_info(targetBucket, key, &manager))
            {
        ...
        for (seq = manager.next_etags_pos + 1; seq <= totalSeq; seq++)
        {
            ...
            do
            {
                // Not necessary, because no data copy needed for this
                /*
                if (srcSize) {
...
/**
 * Searches for an object with the passed @param sourceKey in the @param targetBucket
 * @param targetBucket
 * @param sourceKey
 * @return
 */
char *get_object(const char *targetBucket, const char *sourceKey, size_t *read_size)
{
    const char *bucketName = targetBucket;
    const char *key = sourceKey;
    ...
    char *buf;
    FILE *outputFile;
    outputFile = open_memstream(&buf, read_size);
...
//    S3_init_default();
    ...
    do
    {
        S3_get_object(&bucketContext, key, &getConditions, startByte,
                      byteCount, 0, 0, &getObjectHandler, outputFile);
    } while (
            S3_status_is_retryable(statusG)
            &&
...
/**
 * Searches for the @param key in the @param bucket
 * @param bucket: the bucket to search in
 * @param key: the key to search for
 * @return: existance of @param key in @param bucket
 */
bool head_object(const char *bucket, const char *key)
{
...
int s3copy_main(int argc, char **argv)
{
    // Parse args
    while (1)
    {
    ...
    {
        fprintf(stderr, "s3_test_bucket has been changed and can not be called by parameters anymore!\n");
        // s3_test_bucket(argc, argv, optind);
    } else if (!strcmp(command, "create"))
    {
        fprintf(stderr, "CreateBucket has been changed and can not be called by parameters anymore!\n");
//        create_bucket(argc, argv, optind);
    ...
    } else if (!strcmp(command, "get"))
    {
        ...
        fprintf(stderr, "\nERROR: get_object(..) modified and can't be called from CLI anymore\n");
