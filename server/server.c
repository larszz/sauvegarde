...
static int
process_get_request(server_struct_t *server_struct, struct MHD_Connection *connection, const char *url, void **con_cls);

static json_t *find_needed_hashs(server_struct_t *server_struct, GList *hash_data_list);

static int
answer_meta_json_post_request(server_struct_t *server_struct, struct MHD_Connection *connection, guchar *received_data,
                              guint64 length);

static int answer_hash_array_post_request(server_struct_t *server_struct, struct MHD_Connection *connection,
                                          guchar *received_data);

static void print_received_data_for_hash(guint8 *hash, gssize read);

static int process_received_data(server_struct_t *server_struct, struct MHD_Connection *connection, const char *url,
                                 guchar *received_data, guint64 length);

static guint64 get_header_content_length(struct MHD_Connection *connection, gchar *header, guint64 default_value);

static int
process_post_request(server_struct_t *server_struct, struct MHD_Connection *connection, const char *url, void **con_cls,
...
static int ahc(void *cls, struct MHD_Connection *connection, const char *url, const char *method, const char *version,
...
static server_struct_t *init_server_main_structure(int argc, char **argv)
{
    ...
    if (server_struct->opt != NULL)
    {
        /* meta backend */
        if (server_struct->opt->backend_meta < 0)
        {
            print_error(__FILE__, __LINE__, "Server backend could not be found: Meta\n");
            exit(EXIT_FAILURE);
        } else if (server_struct->opt->backend_meta == BACKEND_UNSET)
        {
            print_error(__FILE__, __LINE__, "Server backend not set: Meta\n");
            exit(EXIT_FAILURE);
        } else if (server_struct->opt->backend_meta == BACKEND_FILE_NUM)
        {
            // use the default file backend
            g_print("Meta Backend: %s\n", BACKEND_FILE_LABEL);
            server_struct->backend_meta = init_backend_structure(file_store_smeta, file_store_data, file_init_backend,
                                                                 NULL,
                                                                 file_build_needed_hash_list, file_get_list_of_files,
                                                                 file_retrieve_data);
        } else if (server_struct->opt->backend_meta == BACKEND_MONGODB_NUM)
        {
            g_print("Meta Backend: %s\n", BACKEND_MONGODB_LABEL);
            server_struct->backend_meta = init_backend_structure(mongodb_store_smeta, NULL, mongodb_init_backend,
                                                                 mongodb_terminate_backend, NULL,
                                                                 mongodb_get_list_of_files, NULL);
        } else
        {
            print_error(__FILE__, __LINE__, "(Internal error) Number of backend to use not handled: %d\n",
                        server_struct->opt->backend_meta);
            exit(EXIT_FAILURE);
        }


        /* optional second explicit data backend */
        // skip if same backend
        if (server_struct->opt->backend_data != server_struct->opt->backend_meta)
        {
            if (server_struct->opt->backend_data < 0)
            {
                print_error(__FILE__, __LINE__, "Server backend could not be found: Data\n");
                exit(EXIT_FAILURE);
            } else if (server_struct->opt->backend_data == BACKEND_UNSET)
            {
                print_error(__FILE__, __LINE__, "Server backend not set: Data\n");
                exit(EXIT_FAILURE);
            } else if (server_struct->opt->backend_data == BACKEND_FILE_NUM)
            {
                // use the default file backend
                g_print("Meta Backend: %s\n", BACKEND_FILE_LABEL);
            ...
            {
                print_error(__FILE__, __LINE__, "(Internal error) Number of backend to use not handled: %d\n",
                            server_struct->opt->backend_data);
                exit(EXIT_FAILURE);
            }
        } else
        {
    ...
    {
        print_error(__FILE__, __LINE__, "Server options missing. Exit.\n");
        exit(EXIT_FAILURE);
    }
...
static gchar *get_a_list_of_files(server_struct_t *server_struct, struct MHD_Connection *connection)
{
    ...
    if (backend->get_list_of_files != NULL)
    {
        ...
        if (query->hostname != NULL && query->uid != NULL && query->gid != NULL && query->owner != NULL &&
            query->group != NULL)
        {
...
static gchar *answer_global_stats(stats_t *stats)
{
    ...
    if (stats != NULL && stats->requests != NULL && stats->requests->get != NULL && stats->requests->post != NULL &&
        stats->requests->unknown != NULL)
    {
        root = json_object();
...
static gchar *get_json_answer(server_struct_t *server_struct, struct MHD_Connection *connection, const char *url)
{
    ...
    } else if (g_str_has_prefix(url, "/Data/"))
    {
        ...
        hash = g_strndup((const gchar *) url + 6,
                         HASH_LEN * 2);  /* HASH_LEN is expressed when hash is in binary form  */
        ...
        {
            message = g_strdup_printf(_("Invalid url: in %s hash has length: %zd instead of %d"), url, hlen,
                                      HASH_LEN * 2);
            answer = answer_json_error_string(MHD_HTTP_BAD_REQUEST, message);
            free_variable(message);
        }
                              ...
                              guint64 length)
{
    ...
    {
        answer = answer_json_error_string(MHD_HTTP_INTERNAL_SERVER_ERROR,
                                          _("Error: could not convert json to metadata\n"));
    }
 ...
 */
static int
answer_hash_array_post_request(server_struct_t *server_struct, struct MHD_Connection *connection, guchar *received_data)
{
    gchar *answer = NULL;         /** gchar *answer : Do not free answer variable as MHD will do it for us !  */
    json_t *root = NULL;          /** json_t *root is the root that will contain all meta data json formatted */
    json_t *array = NULL;         /** json_t *array is the array that will receive base64 encoded hashs       */
 ...
 */
static int
answer_data_post_request(server_struct_t *server_struct, struct MHD_Connection *connection, guchar *received_data)
{
    gchar *answer = NULL;                   /** gchar *answer : Do not free answer variable as MHD will do it for us ! */
    hash_data_t *hash_data = NULL;
    int success = MHD_NO;
 ...
 */
static int
answer_data_array_post_request(server_struct_t *server_struct, struct MHD_Connection *connection, guchar *received_data)
{
    gchar *answer = NULL;                   /** gchar *answer : Do not free answer variable as MHD will do it for us ! */
    hash_data_t *hash_data = NULL;
    int success = MHD_NO;
    ...
    if (server_struct->meta_queue != NULL)
    {
        ...
        } else
        {
            print_error(__FILE__, __LINE__,
                        _("Error: no meta data store backend defined, meta-data's thread terminating...\n"));
        }
    } else
    {
...
int main(int argc, char **argv)
{
        ...
        server_struct->backend_meta != NULL)
    {
        ...
        server_struct->d = MHD_start_daemon(MHD_USE_THREAD_PER_CONNECTION | MHD_USE_DEBUG, server_struct->opt->port,
                                            NULL, NULL, &ahc, server_struct, MHD_OPTION_CONNECTION_MEMORY_LIMIT,
                                            (size_t) 131070, MHD_OPTION_CONNECTION_TIMEOUT, (unsigned int) 120,
                                            MHD_OPTION_END);
